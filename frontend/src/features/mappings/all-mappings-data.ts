...
export const ALL_MAPPINGS: AllMappingListItem[] = [
  {
    id: 'mapping-1',
    index: 1,
...
export const MAPPING_STATUS_SUMMARY = {
  total: ALL_MAPPINGS.length,
  complete: ALL_MAPPINGS.filter((item) => item.status === 'Complete').length,
  partial: ALL_MAPPINGS.filter((item) => item.status === 'Partial').length,
  draft: ALL_MAPPINGS.filter((item) => item.status === 'Draft').length,
};
