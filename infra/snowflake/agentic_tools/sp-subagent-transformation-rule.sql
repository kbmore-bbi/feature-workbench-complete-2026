CREATE OR REPLACE PROCEDURE __STTM_METADATA_NAMESPACE__.SP_SUBAGT_TRANSFORMATION_RULE("QUERY_CONTEXT" VARCHAR, "MODEL_NAME" VARCHAR DEFAULT 'claude-haiku-4-5')
RETURNS VARCHAR
LANGUAGE PYTHON
...
def run_agent(session, query_context: str, model_name: str = ''claude-haiku-4-5'') -> str:
    ...
    response = _snowflake.send_snow_api_request(
        ''POST'',
        f''/api/v2/databases/{current_database}/schemas/{current_schema}/agents/AGT_TRANSFORMATION_RULE:run'',
        {},
        {},
        payload,
        None,
