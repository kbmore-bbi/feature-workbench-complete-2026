...
def generate(
    ...
    job_id, total = service.submit(body)
    background_tasks.add_task(service.run_generation, job_id, body, user_token)
    return GenerateResponse(
        job_id=job_id,
        total=total,
        message=f"Queued {total} tasks for {len(body.tables)} table(s)",
    )
...
@router.get(
    "",
    response_model=SemanticModelRecord,
...
def get_semantic_model(
    ...
    return SemanticModelRecord(**record)
